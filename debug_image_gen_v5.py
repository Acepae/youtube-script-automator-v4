...
def log(msg):
    with open("gen_debug_v2.txt", "a", encoding="utf-8") as f:
        f.write(msg + "\n")
    print(msg)

if __name__ == "__main__":
    if os.path.exists("gen_debug_v2.txt"): os.remove("gen_debug_v2.txt")
    
    api_key = get_api_key()
    genai.configure(api_key=api_key)
    
    prompt = "Generate an image of a cute cat, 2k resolution, aspect ratio 16:9"
    
    models_to_test = [
        "models/gemini-2.0-flash-exp-image-generation",
        "models/gemini-2.5-flash-image",
        "models/imagen-3.0-generate-001" # Just in case it works via generate_content wrapper
    ]
